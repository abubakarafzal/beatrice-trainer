...
def prepare_training():
    ...
    # Adjust DataLoader settings for small datasets and Windows
    n_samples = len(training_dataset)
    effective_batch_size = min(h.batch_size, n_samples if n_samples > 0 else h.batch_size)
    drop_last_flag = n_samples >= h.batch_size
    training_loader = torch.utils.data.DataLoader(
        training_dataset,
        # On Windows, classes defined in __main__ cannot be pickled by worker processes.
        # Force single-process data loading and disable persistent workers to avoid crashes.
        num_workers=0 if os.name == "nt" else min(h.num_workers, os.cpu_count()),
